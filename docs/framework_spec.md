...
## Perturbation-validation data (scout 2026-07-14 — PARTIAL, scout stalled mid-run)
- **Replogle genome-scale Perturb-seq (K562, CRISPRi)** — perturbs **GATA1 / TAL1 / LMO2** (+~9,800
  genes). Direct **Erythropoiesis** perturbation validation (K562 = erythroleukemia; those are our
  erythroid KG nodes), and it **pairs with the erythroid SHARE-seq multiome** → emergence *and*
  perturbation on the same program. **Top validation target.** [accession to confirm — scout stalled]
- **PENDING (scout must be re-run):** cardiomyocyte **HDAC4/5-knockdown or CaMKII-inhibitor** single
  cell — the FLAGSHIP unconfirmed hypertrophy prediction; TAC cardiomyocyte perturbation. Not yet found.
- Broadly: any Perturb-seq/CROP-seq or drug/cue control-vs-treated single-cell whose perturbed target
  is a KG-cascade node (EMT TNFα/TGFβ/Twist1; ADM NF-κB; macrophage LPS; reprogramming OSKM; osteo YAP/TAZ).
